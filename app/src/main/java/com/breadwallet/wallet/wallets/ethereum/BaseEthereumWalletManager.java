            ...
            for (int i = txs.length - 1; i >= 0; i--) { //revere order
                ...
                if (tx.isSubmitted()) {
                    ...
                    uiTxs.add(new TxUiHolder(tx, tx.getTargetAddress().equalsIgnoreCase(getEthereumWallet().getWallet().getAccount().getPrimaryAddress()),
                            tx.getBlockTimestamp(), (int) tx.getBlockNumber(), Utils.isNullOrEmpty(tx.getIdentifier())
                            ? null : tx.getIdentifier().getBytes(), tx.getIdentifier(), new BigDecimal(tx.getFee(feeUnit)),
